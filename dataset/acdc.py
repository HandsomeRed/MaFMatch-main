...
class ACDCDataset(Dataset):
    ...
    def __getitem__(self, item):
        id = self.ids[item] # data/slices/patient003_frame02_slice_6.h5
        ...
        if self.mode == 'test':
            return torch.from_numpy(img).float(), torch.from_numpy(mask).long(), os.path.join(self.root, id)
        ...
        cutmix_box1 = obtain_cutmix_box(self.size, p=0.5)  # cutmix_box中有0和1两个值，为1的区域是一个矩形区域，可以根据这个区域来mix两张图像
...
if __name__ == '__main__':
    trainset_l = ACDCDataset('acdc', 'G:\Ai\dataset\ACDC', 'train_l',256, 'G:/Ai/code/deep_learning/image_segmentation/UniMatch-main/more-scenarios/medical/splits/acdc/7/labeled.txt',1176)
